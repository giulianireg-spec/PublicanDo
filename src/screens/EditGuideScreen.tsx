// src/screens/EditGuideScreen.tsx
// Pantalla para editar guías existentes en GuianDo
...
import { getGuideById, updateGuide, searchPlacesForGuide } from '../services/guidesApi';
...
interface PointForm {
  ...
  customImageUrl?: string; // URL existente de Cloudinary
  location: {
    address: string;
    city: string;
    province: string;
...
const EditGuideScreen = ({ navigation, route }: any) => {
...
const styles = StyleSheet.create({
  ...
  const [coverImageUrl, setCoverImageUrl] = useState<string | null>(null); // URL existente
  ...
  const loadGuide = async () => {
    try {
      ...
      setCategory(guide.category as GuideCategory);
      setDifficulty(guide.difficulty as GuideDifficulty);
      setCoverImageUrl(guide.coverImageUrl);
      
      // Convertir puntos al formato del formulario
      const formPoints: PointForm[] = guide.points.map((p: any) => ({
        type: p.type,
        placeId: p.placeId?._id || p.placeId,
        placeData: p.placeId && typeof p.placeId === 'object' ? p.placeId : null,
        ...
        location: {
          ...
          province: p.location?.province || 'Córdoba',
      ...
      setPoints(formPoints.length >= 2 ? formPoints : [...formPoints, { ...emptyPoint }]);
    } catch (error) {
  ...
  const pickCoverImage = async () => {
    ...
    if (!result.canceled && result.assets[0]) {
      ...
      setCoverImageUrl(null); // Limpiar URL existente
    }
  };
  ...
  const validateForm = (): string | null => {
    ...
    if (!coverImageBase64 && !coverImageUrl) return 'Selecciona una imagen de portada';
    ...
    try {
      const guideData: any = {
        ...
        points: points.map((p, index) => ({
          ...
          customImageBase64: p.type === 'custom' && p.customImageBase64 ? p.customImageBase64 : undefined,
          customImageUrl: p.type === 'custom' && !p.customImageBase64 ? p.customImageUrl : undefined,
          location: {
            address: p.location.address,
            city: p.location.city,
      ...
      if (coverImageBase64) {
        guideData.coverImageBase64 = coverImageBase64;
      }
      ...
      Toast.show({
        type: 'error',
        text1: 'Error',
        text2: error.response?.data?.message || 'No se pudo actualizar la guía',
        position: 'bottom',
      });
    } finally {
      setSubmitting(false);
  ...
  const renderPoint = (point: PointForm, index: number) => {
    const isPlace = point.type === 'place' && (point.placeData || point.placeId);
    const pointImage = point.customImageUri || point.customImageUrl;
      ...
      <View key={index} style={styles.pointCard}>
        <View style={styles.pointHeader}>
          ...
          <Text style={styles.pointTitle}>
            {isPlace ? (point.placeData?.title || 'Lugar vinculado') : point.customName || 'Punto personalizado'}
          </Text>
        ...
        {isPlace ? (
          <View style={styles.placePreview}>
            {point.placeData?.imageUrl && (
              <Image source={{ uri: point.placeData.imageUrl }} style={styles.placePreviewImage} />
            )}
            <View style={styles.placePreviewInfo}>
              <Text style={styles.placePreviewName}>{point.placeData?.title || 'Lugar vinculado'}</Text>
              <Text style={styles.placePreviewLocation}>
                {point.location?.city}, {point.location?.province}
              </Text>
            </View>
            <TouchableOpacity onPress={() => {
              setEditingPointIndex(index);
          ...
          <View style={styles.customPointForm}>
            ...
            >
              {pointImage ? (
                <Image source={{ uri: pointImage }} style={styles.pointImagePreview} />
              ) : (
            ...
            <View style={styles.locationRow}>
              <View style={styles.locationField}>
                ...
                <TextInput
                  ...
                  value={point.location.province}
                  onChangeText={(text) => updatePoint(index, {
                    location: { ...point.location, province: text }
                  })}
                />
              </View>
              <View style={styles.locationField}>
  ...
  return (
    ...
    >
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Editar guía</Text>
        <TouchableOpacity 
      ...
      <ScrollView ref={scrollRef} style={styles.scrollView} keyboardShouldPersistTaps="handled">
        ...
        <TouchableOpacity style={styles.coverPicker} onPress={pickCoverImage}>
          {coverImageUri || coverImageUrl ? (
            <Image source={{ uri: coverImageUri || coverImageUrl! }} style={styles.coverImage} />
          ) : (
        ...
        <View style={styles.section}>
          ...
          >
            <Text style={styles.categoryIconText}>{getGuideCategoryConfig(category).icon}</Text>
